#!/usr/bin/env python3
"""PARAMETRIC version of the HL PickCube phase controller for experiment #2.
...
from mujoco.mjx._src import math as _mjxmath
import os as _os

# Orientation-aware switch: if RES_ORI_AWARE=1, the analytic controller rotates
# the desired end-effector frame by the per-episode TARGET rotation so the
# rigidly-grasped cube ENDS at the target orientation. Default off reproduces v9.
ORI_AWARE = _os.environ.get("RES_ORI_AWARE", "0") == "1"

# Part 34 lever A (EARLY rotation): apply R_target rotation from an EARLIER phase
# so the gripper PRE-ALIGNS before grasping (instead of only from GRASP onward).
# RES_ROT_FROM in {APPROACH,DESCEND,GRASP}; default GRASP reproduces Part 33.
_ROT_FROM_NAME = _os.environ.get("RES_ROT_FROM", "GRASP").upper()
_ROT_FROM_MAP = {"APPROACH": 0, "DESCEND": 1, "GRASP": 2}
ROT_FROM = _ROT_FROM_MAP.get(_ROT_FROM_NAME, 2)
...
class ParamPick:
    ...
    def __init__(self, env):
        ...
        # per-episode target-orientation source (mocapid of the target body)
        self.mocap_target = env._mocap_target
    ...
    def act(self, state, cstate, kv):
        ...
        # ORIENTATION-AWARE desired EE frame: rotate R_home by the per-episode
        # target rotation so the rigidly-grasped cube ends at the target orient.
        # Applied from GRASP onward (when the cube is held); pre-grasp stays level.
        if ORI_AWARE:
            R_target = _mjxmath.quat_to_mat(
                state.data.mocap_quat[self.mocap_target].reshape(4)).reshape(3, 3)
            # Part 34: apply rotation from ROT_FROM phase onward (default GRASP).
            # Earlier (APPROACH/DESCEND) pre-aligns the gripper before contact.
            apply_rot = phase >= ROT_FROM
            R_des = jp.where(apply_rot, R_target @ self.R_home, self.R_home)
        else:
            R_des = self.R_home
        # use_aik fixed on:
        q_aik, aik_ok = aik_solve(goal_rl, R_des, q_arm,
