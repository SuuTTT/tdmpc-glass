...
import param_controller_oriaware_v2 as PC
from mujoco.mjx._src import math as _mjxmath
import os as _os
# orientation-aware obs conditioning: append the per-episode target quaternion (4)
# so the learned residual is Markov in the absolute target orientation, not just
# the relative rot-error already present in the 66-dim base obs.
ORI_AWARE = _os.environ.get("RES_ORI_AWARE", "0") == "1"
_N_ORI = 4 if ORI_AWARE else 0

# Part 34 lever B (richer conditioning): also append the so(3) rotation ERROR
# between the current cube orientation and the target orientation (3 dims), giving
# the residual a direct, bounded "how far from target rot" signal each step.
ROTERR_OBS = _os.environ.get("RES_ROTERR_OBS", "0") == "1"
_N_ROTERR = 3 if ROTERR_OBS else 0
...
class ResidualPickCube(PandaPickCube):
  ...
  def _aug_obs(self, base_obs, state, cs):
    ...
    parts = [base_obs, onehot, subgoal, grip]
    if ORI_AWARE:
      tq = state.data.mocap_quat[self._mocap_target].reshape(-1)  # (4,) target orient
      parts.append(tq)
    if ROTERR_OBS:
      # so(3) error between current cube R and target R: e = vee(Rt^T Rc - Rc^T Rt)/2
      Rt = _mjxmath.quat_to_mat(
          state.data.mocap_quat[self._mocap_target].reshape(4)).reshape(3, 3)
      Rc = _mjxmath.quat_to_mat(
          state.data.xquat[self._obj_body].reshape(4)).reshape(3, 3)
      M = Rt.T @ Rc
      roterr = 0.5 * jp.array([M[2, 1] - M[1, 2],
                               M[0, 2] - M[2, 0],
                               M[1, 0] - M[0, 1]])  # (3,)
      parts.append(roterr)
  ...
  def observation_size(self):
    return 66 + _N_PHASES + 3 + 1 + _N_ORI + _N_ROTERR  # 77 (+4 quat, +3 roterr)
