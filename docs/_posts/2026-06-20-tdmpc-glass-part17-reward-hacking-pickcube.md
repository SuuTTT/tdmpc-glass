...
pattern** — `jumpy_reached` peaks 0.33 early via macro-step planning, then **fades to 0 by ~400k**,
`box_target` ≤ 0.012, **success 0** (through 500k) — i.e. temporal abstraction does **not** cross the
grasp-stability wall either. So *every*
...
null is settled — fade-to-0 by 400k matches every other variant.)*
