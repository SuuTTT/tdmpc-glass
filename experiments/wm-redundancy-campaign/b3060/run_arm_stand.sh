#!/usr/bin/env bash
# WM-head-ablation single-arm runner, HopperStand variant (task-suffixed tags so it
...
TAG=wmablstand_${ABLATE}_s${SEED}
...
echo "[run_arm_hop] START ablate=$ABLATE seed=$SEED gpu=$GPU steps=$TOTAL_STEPS task=$TASK tag=$TAG $(date -u +%FT%TZ)" | tee -a "$LOG"
$PY -u scripts/run_benchmark.py --algos tdmpc2 --tasks "$TASK" \
  ...
  --no_plot >> "$LOG" 2>&1
rc=$?
echo "[run_arm_hop] DONE ablate=$ABLATE seed=$SEED rc=$rc $(date -u +%FT%TZ)" | tee -a "$LOG"
exit $rc
