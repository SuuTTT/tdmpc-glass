...
# ADDITIVE (CompPlan/GHM unblock): InFOM's '-ft-' finetuning datasets are not on the OGBench
# server for antmaze (the generator only covers cube/scene/puzzle). When this flag is set, the
# finetuning dataset is loaded from the SAME (non-ft) reward-labeled singletask dataset used for
# pretraining, so that the eval loop (which only runs during the finetuning phase) can execute
# without a separate '-ft-' dataset. Reward labels are still present (OGBench relabels any
# singletask dataset), so finetuning + eval (episode.success) run end-to-end. Default 0 keeps the
# original InFOM behavior untouched. Auto-enabled below for antmaze envs.
flags.DEFINE_integer('ft_dataset_fallback', 0,
                     'Use the non-ft (pretraining) singletask dataset as the finetuning dataset.')
...
def main(_):
    ...
    # ADDITIVE: auto-enable the ft-dataset fallback for antmaze (no '-ft-' dataset exists on the
    # OGBench server, and InFOM's generator does not cover antmaze). The fallback loads the non-ft
    # singletask dataset (still reward-labeled) as the finetuning dataset so eval can run.
    ft_fallback = bool(FLAGS.ft_dataset_fallback) or ('antmaze' in FLAGS.env_name)
    # When falling back, load the finetuning dataset from the non-ft path (reward_free=True path
    # builds the dataset name WITHOUT '-ft-'; OGBench still relabels singletask rewards).
    _, eval_env, finetuning_train_dataset, finetuning_val_dataset = make_env_and_datasets(
        ...
        reward_free=ft_fallback)
    if ft_fallback:
        print(f'[ghm-unblock] ft_dataset_fallback active for {FLAGS.env_name}: '
              f'finetuning dataset = non-ft singletask dataset (reward-labeled).')
    ...
    for dataset in [pretraining_train_dataset, pretraining_val_dataset,
        ...
        if dataset is not None:
            ...
            if config['agent_name'] in ['infom', 'ghm', 'rebrac', 'dino_rebrac', 'mbpo_rebrac',
    ...
    for i in tqdm.tqdm(range(1, FLAGS.pretraining_steps + FLAGS.finetuning_steps + 1), smoothing=0.1, dynamic_ncols=True):
        if i <= FLAGS.pretraining_steps:
            ...
            if i == (FLAGS.pretraining_steps + 1):
                if config['agent_name'] in ['infom', 'ghm', 'dino_rebrac', 'td_infonce', 'hilp']:
                    agent.target_reset()
