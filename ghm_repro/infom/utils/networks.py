...
class HorizonVectorField(nn.Module):
    """Horizon- (and optionally policy-) conditioned flow-matching vector field for GHM.

    Identical to `VectorField` but accepts two extra conditioning inputs:
        - `horizons`: a per-example geometric-horizon discount gamma_h in (0, 1), fed as an extra
          channel (optionally Fourier-embedded) so a single model answers all horizons (CompPlan/TD-Flow).
        - `policy_embeds`: an optional per-example policy embedding e_k (CompPlan policy conditioning).
    Keeping it as a separate module means the existing InFOM `VectorField` is untouched.

    Attributes:
        vector_dim: Output (data) dimension.
        hidden_dims: Hidden layer dimensions.
        layer_norm: Whether to apply layer normalization.
        num_ensembles: Number of ensemble components.
        encoder: state encoder.
        horizon_fourier_dim: If > 0, Fourier-embed the horizon scalar to this many features (must be even).
                             If 0, feed the raw scalar gamma_h (cheapest, mirrors how flow-time `t` is fed).
    """
    ...
    def _horizon_features(self, horizons):
        """horizons: (...,) scalar gamma_h -> (..., F) features."""
        h = horizons[..., None]
        if self.horizon_fourier_dim and self.horizon_fourier_dim > 0:
            # Fixed log-spaced frequencies; embed gamma_h with sin/cos (no learned params here).
            half = self.horizon_fourier_dim // 2
            freqs = jnp.exp(jnp.linspace(0.0, jnp.log(1000.0), half))  # (half,)
            ang = h * freqs  # broadcast -> (..., half)
            return jnp.concatenate([jnp.sin(ang), jnp.cos(ang)], axis=-1)
        return h

    def __call__(self, noisy_goals, times, observations=None, actions=None, latents=None,
                 horizons=None, policy_embeds=None):
        """Return the horizon-conditioned velocity field.

        Args mirror `VectorField` plus:
            horizons: per-example geometric-horizon discount gamma_h (broadcastable to leading dims).
            policy_embeds: optional per-example policy embedding (CompPlan policy conditioning).
        """
        if self.encoder is not None:
            noisy_goals = self.encoder(noisy_goals)
        if self.encoder is not None and observations is not None:
        ...
        if horizons is not None:
            inputs.append(self._horizon_features(horizons))
        if policy_embeds is not None:
            inputs.append(policy_embeds)
        inputs = jnp.concatenate(inputs, axis=-1)
