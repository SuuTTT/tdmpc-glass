...
# Phase ids
APPROACH, DESCEND, GRASP, LIFT, HOLD = 0, 1, 2, 3, 4
...
class Knobs:
    ...
    lift_clear: float = 0.10        # LIFT: extra z above box before transporting
    place_tol: float = 0.020        # LIFT->HOLD: box-target distance to call placed
...
class HeuristicPick:
    ...
    def act(self, state, cstate):
        ...
        # LIFT: raise then transport box to target (closed-loop on box error).
        err_bt = target - box
        g_lift = site + err_bt
        # ensure we clear first: if box not lifted yet, just go up
        not_up = box[2] < (0.03 + k.lift_clear * 0.5)
        g_lift = jp.where(
            not_up,
            jp.array([lock_xy[0], lock_xy[1], box[2] + k.lift_clear + 0.03]),
            g_lift,
        )
        g_hold = site + err_bt
        ...
        held = closed | (grip_cmd_new <= self.grip_lo + 1e-4)
        to_lift = (phase == GRASP) & (((timer > k.grasp_hold_steps) & held)
                                      | (timer > k.grasp_max))

        placed = jp.linalg.norm(err_bt) < k.place_tol
        to_hold = (phase == LIFT) & placed

        next_phase = jp.select(
            [to_descend, to_grasp, to_lift, to_hold],
            [jp.int32(DESCEND), jp.int32(GRASP), jp.int32(LIFT), jp.int32(HOLD)],
            default=phase,
        )
