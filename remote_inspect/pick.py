  ...
  config = config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              ...
              gripper_box=float(os.environ.get("PICKCUBE_W_GRIPPER_BOX", "4.0")),
              # Box goes to the target mocap.
              box_target=float(os.environ.get("PICKCUBE_W_BOX_TARGET", "8.0")),
              # Do not collide the gripper with the floor.
              no_floor_collision=0.25,
              # Arm stays close to target pose.
              robot_target_qpos=0.3,
              # REWARD-ENG (additive): ungated lift bonus + grasp latch bonus.
              # Default 0.0 => identical to original reward. Set via env vars.
              lift_bonus=float(os.environ.get("PICKCUBE_W_LIFT", "0.0")),
              grasp_bonus=float(os.environ.get("PICKCUBE_W_GRASP", "0.0")),
          )
      ),
      impl='warp',
      naconmax=24 * 2048,
...
class PandaPickCube(panda.PandaBase):
  ...
  def _get_reward(self, data: mjx.Data, info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prev_reached = info["reached_box"]
    info["reached_box"] = 1.0 * jp.maximum(
        info["reached_box"],
        (jp.linalg.norm(box_pos - gripper_pos) < 0.012),
    )

    # REWARD-ENG (additive shaping; does NOT touch box_target metric).
    # Ungated lift bonus: cube height above its resting table z, in [0,1] via
    # tanh saturating at ~20cm. Gives "pick it up" a gradient before a perfect
    # grasp/place, breaking the hover local optimum.
    lift = jp.clip(box_pos[2] - self._init_obj_pos[2], 0.0, None)
    lift_bonus = jp.tanh(5.0 * lift)
    # Grasp latch bonus: one-shot-ish reward the instant reached_box flips 0->1,
    # bridging the gated cliff. (Step reward while newly-reached this step.)
    grasp_bonus = info["reached_box"] * (1.0 - prev_reached)
