#!/usr/bin/env bash
# run_dreamer_baseline.sh — queue launcher for the standalone DreamerV3 driver
# (scripts/run_dreamer.py). Mirrors run_dmc_baseline.sh's env-var convention so
# the task_queue_daemon can launch DreamerV3-vs-TD-MPC2 comparison runs with the
# same env-string shape (PROBE_ID, ALGO, TASK, SEEDS, TOTAL_STEPS, CODE_SHA).
#
# Does NOT touch the hot path (run_benchmark.py / tdmpc2.py / tdmpc_glass.py).
# CSVs land under exp/tdmpc_glass/<TASK>[_<TAG>]/seed_<seed>.csv (eval_type=dreamer)
# plus a rollup at exp/benchmark/dreamer_<TASK>[_<TAG>].csv.
set -u; set +e
...
PROBE_ID=${PROBE_ID:-dreamer_baseline}
ALGO=${ALGO:-dreamer}            # accepted for parity; this launcher only runs dreamer
...
{
  echo "[${PROBE_ID}] start $(date -u +%FT%TZ) algo=dreamer task=${TASK} seeds=${SEEDS} sha=${CODE_SHA} tag=${TDMPC_GLASS_OUTPUT_TAG}"
...
for seed in $SEEDS; do
  ...
  python3 -u scripts/run_dreamer.py \
